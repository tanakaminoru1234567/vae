...
class SeqVAE(chainer.Chain):
    ...
    def __init__(self, w_input, n_latent, n_filter, ksize, stride, n_h2):
        super(SeqVAE, self).__init__()
        self.n_filter = n_filter
        self.w_hidden = (w_input - ksize) // stride + 1
        n_hidden = n_filter * self.w_hidden * self.w_hidden
        with self.init_scope():
            # encoder
            self.le1 = L.Convolution2D(1, n_filter, ksize, stride)
            self.le2 = L.Linear(n_hidden, 2 * n_latent)
            # decoder
            self.ld1 = L.Linear(n_latent, n_hidden)
            self.ld2 = L.Deconvolution2D(n_filter, n_filter, ksize, stride)
